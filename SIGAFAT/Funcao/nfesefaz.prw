...
Local cBarra 	    := ""
Local cBarTrib 	    := ""
Local cFCI          := "" // DUX PERSONALIZAÇÃO FCI 
					...
					If lContinua
						...
						If (cOrigem $"1-2-3-4-5-6-8" .And. (cCSTrib $ "00-10-20-30-40-41-50-51-60-70-90" .or. cCsosn2 $ "101-102-103-201-202-201-300-400-500-900"))
							If (cAliasSD2)->(FieldPos("D2_FCICOD")) > 0 .And. !Empty((cAliasSD2)->D2_FCICOD)
								...
								If lFCI
									...
									cFCI := Alltrim((cAliasSD2)->D2_FCICOD) // DUX PERSONALIZAÇÃO FCI 
								EndIf
						...
						*/
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							"S",;						//aprod[56]
							cFCI;                       //aprod[57] -- DUX PERSONALIZAÇÃO FCI
							})
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "PS2"
									If !lNfCupZero
										...
										EndIf
									Else
										aTail(aPIS) := {SF4->F4_CSTPIS,0,0,0,CD2->CD2_QTRIB,CD2->CD2_PAUTA,CD2->CD2_CODPRO} // Dux - Personalização - Para Tratar no PE
									EndIf
								Case AllTrim(CD2->CD2_IMP) == "CF2"
									If !lNfCupZero
										...
										EndIf
									Else
										aTail(aCOFINS) := {SF4->F4_CSTCOF,0,0,0,CD2->CD2_QTRIB,CD2->CD2_PAUTA,CD2->CD2_CODPRO} // Dux - Personalização - Para Tratar no PE
									EndIf
								Case AllTrim(CD2->CD2_IMP) == "PS3" .And. (cAliasSD2)->D2_VALISS==0
									...
									Else
										aTail(aPISST) := {SF4->F4_CSTPIS,0,0,0,CD2->CD2_QTRIB,CD2->CD2_PAUTA,CD2->CD2_PSCFST,CD2->CD2_CODPRO} // Dux - Personalização - Para Tratar no PE
									EndIf
								...
								Case AllTrim(CD2->CD2_IMP) == "CF3" .And. (cAliasSD2)->D2_VALISS==0
										...
										Else
											aTail(aCOFINSST) := {SF4->F4_CSTCOF,0,0,0,CD2->CD2_QTRIB,CD2->CD2_PAUTA,CD2->CD2_PSCFST,CD2->CD2_CODPRO} // Dux - Personalização - Para Tratar no PE
										EndIf
								...
								Case AllTrim(CD2->CD2_IMP) == "ISS" 
									...
									aTail(aISSQN) := {CD2->CD2_BC,CD2->CD2_ALIQ,CD2->CD2_VLTRIB,cMunISS,cCodIss,cSitTrib,nDeducao,cIndIss,nValISSRet,CD2->CD2_CODPRO} // Dux - Personalização - Para Tratar no PE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-30-40-41-50-51-60-70-90")
					If (cAliasSD1)->(ColumnPos("D1_FCICOD")) > 0 .And. !Empty((cAliasSD1)->D1_FCICOD)
						...
						If lFCI
							...
							cFCI := Alltrim((cAliasSD1)->D1_FCICOD)  // DUX PERSONALIZAÇÃO - FCI 
						EndIf					
					Else
						aadd(aFCI,{})
					EndIf
				...
				*/				
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					"E",;                       //aProd[56]
					cFCI;						//aProd[57] --- DUX PERSONALIZAÇÃO 
					})
						...
						Case AllTrim(CD2->CD2_IMP) == "SOL"
							...
							If !lIcmSTDev
								If ( (cAliasSD1)->D1_TIPO=="D" .Or. ( (cAliasSD1)->D1_TIPO=="I" .And. lComplDev)) .And. !Empty(nValST) 
									...
									0,;
									IIf(SFT->(ColumnPos("FT_MOTICMS")) > 0,IIF(SF1->(F1_STATUS) == 'C',SF4->F4_MOTICMS ,SFT->FT_MOTICMS),"");
									} // Dux - Personalização - Para Tratar no PE 
								EndIf
							EndIf
...
IF lPe01Nfe
/* DUX | Personalização
	aICMS,;    		//21 | DUX | Personalização 
	aIPI,;     		//22 | DUX | Personalização
	aICMSST,;  		//23 | DUX | Personalização
	aPIS,;     		//24 | DUX | Personalização
	aPISST,;   		//25 | DUX | Personalização
	aCOFINS,;  		//26 | DUX | Personalização
	aCOFINSST,;		//27 | DUX | Personalização
	aISSQN,;		//28 | DUX | Personalização
	aTotal,;		//29 | DUX | Personalização
	aICMSZFM,;		//30 | DUX | Personalização
	aICMUFDest;		//31 | DUX | Personalizao
	aCST }          // 32 | DUX | Personalizao - Allan
	aFCI           // 33 | DUX | Personalizao - Allan
*/	
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	*/
	...
	EndIf
	if !lBrinde
		cString += NfeTag('<EMAIL>',AllTrim(aDest[16])+cMailTrans+';dux.centraliza@saastargetit.com.br') //DUX | Personalização
	endif
...
If len(aNota) > 1 .and. MsSeek(_cFilial+cDoc+cSerie+cCliente+cLoja+cCodigo+cItem)
