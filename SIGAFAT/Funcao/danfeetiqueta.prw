...
user function ImpDfEtq(cUrl, cIdEnt, lUsaColab)
    ...
    begin sequence
        ...
        // Dux - Custom - Inicio 
        if(empty(cChaveNfe))
            IF  !("ADFATAUT" $ FUNNAME() ) .And. !(IsInCallStack("U_TFATA02") .Or. "TFATA02" $ FunName())
    ...
    cLocImp := MV_PAR08 // Impressora 
        ...
        // Dux - Custom - Inicio
		if(!empty(cChaveNfe))
			cWhere += " AND SF3.F3_CHVNFE IN "+formatIn(cChaveNfe,"|")
		endif
		// Dux - Custom - Fim
        ...
        for nNotas := 1 to len(aNotas)
            ...
            if (!empty(cProtocolo) .or. !empty(cDpecProt))
                ...
                if nTipoDoc == 2
                    ...
                    // Dux - Custom - Inicio
                    if(!empty(cChaveNfe))
                        if(!(alltrim(SF2->F2_CHVNFE) $ cChaveNfe))
                            loop
                        endif
                    endif
                    //Dux - Custom - Fim
