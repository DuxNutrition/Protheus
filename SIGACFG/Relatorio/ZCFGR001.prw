...
Static Function zRel0001(cArquivo)
	...
	If (cTmpUsr)->(!Eof())
		...
		While (cTmpUsr)->(!EoF())
            ...
            cQryGrp := ""
            cQryGrp += " SELECT USR_GRUPO FROM SYS_USR_GROUPS "             + CRLF
	        ...
	        If (cTmpGrp)->(!Eof())
                While (cTmpGrp)->(!Eof())
                    ...
                    cQryMod := ""
                    cQryMod += " SELECT GR__ID, GR__ACESSO, GR__MODULO FROM SYS_GRP_MODULE "    + CRLF
            ...
            cQryUsM := ""
            cQryUsM += " SELECT USR_MODULO FROM SYS_USR_MODULE "        + CRLF
...
cQry := ""
cQry += " SELECT IDGRUPO.USR_GRUPO, ACGRUPO.GR__ID, ACGRUPO.GR__CODIGO, ACGRUPO.GR__NOME, ACGRUPO.GR__TIMEOUT  FROM SYS_USR_GROUPS IDGRUPO "    + CRLF
cQry += "     INNER JOIN SYS_GRP_GROUP ACGRUPO "                                                                                                + CRLF
