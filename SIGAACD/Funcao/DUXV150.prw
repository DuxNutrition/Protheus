...
User Function DUXV150(lEndCQ)
	...
	Private cObs		:= Space(Tamsx3("ZD3_OBS") [1])
	...
	While .t.
		VTClear
		nLin:= -1
		@ ++nLin,0 VtSay "Embarque"  //"Transferencia"
		If !lVolta
			If ! GetObs()
				Exit
			EndIf
...
Static Function GetObs()
	Local lRet := .T.
	If ! UsaCB0('01')
		@ ++nLin,0 VtSay "Informe uma Obs." //'Endereco origem'
		@ ++nLin,0 VTGet cObs pict "@!" Valid ! Empty(cObs)  when Empty(cObs)
	...
	If ! SBE->(MsSeek(xFilial("SBE")+cArmazem+cEndereco))
		...
		elseIf Empty(cEndereco) .and. !Empty(cArmazem)
			If NNR->(MsSeek(xFilial("NNR")+cArmazem))
				...
				lRet := .F.
				VtAlert("armazem n„o localizado", STR0013,.t.,4000,4)// armazem n„o localizado
			EndIf
		else
			VtAlert(STR0007, STR0013,.t.,4000,4) //'Endereco nao encontrado'###'Aviso'
			VTKeyboard(chr(20))
			...
			Begin Sequence
				For nP := 1 to len(aItensPallet)
					...
					If lWmsNew .And. IntWms(aEtiqueta[1])
						VTALERT("WMS",STR0013, .T.)//"Para o Novo WMS e Produtos com controle WMS usar: Menu Atualizacoes>Estoque WMS>Transferencia WMS"
						Break
					EndIf
		...
		Begin Sequence
			For nP := 1 to len(aItensPallet)
				...
				If lTrfLote .And. !Empty(cLote)
					If	VTYesNo("Confirma transferencia?",STR0013 ,.T.) //"Confirma transferencia?"###"Aviso"
			...
			For nI := 1 to Len(aLista)
				...
				If Rastro(aLista[nI,1])
					...
					If !lConfirma .And. !Empty(dValidDest) .And. dValidDest != dValid
						VTALERT("Existe(m) produto(s) com data de validade diferente no armazem de destino",STR0013,.T.,NIL,3) // "Existe(m) produto(s) com data de validade diferente no armazem de destino"
						If !VTYesNo("Sera assumida a data de validade do armazem de destino. Confirmar operacao?","Lote Destino",.t.)
							VTALERT("Processo cancelado",STR0013,.T.,NIL,3) // "Processo cancelado"
							DisarmTransaction()
							Break
						Else
							lConfirma := .T.
...
Static Function GravaZD3()

	Local aZD3			:= ZD3->(GetArea())
	Local nX := 0

    Local cCod := GetSxeNum("ZD3","ZD3_DOCMOV")
	...
	for nX := 1 to len(aLista)
		...
		ZD3->ZD3_PROD   := aLista[nX,1]
		ZD3->ZD3_LOCORI	:= aLista[nX,3]
		ZD3->ZD3_ENDORI := aLista[nX,4]
		ZD3->ZD3_LOTORI := aLista[nX,5]
		ZD3->ZD3_QTDORI := aLista[nX,2]
        ZD3->ZD3_LOCDES := cArmDes
        ZD3->ZD3_ENDDES := cEndDes
